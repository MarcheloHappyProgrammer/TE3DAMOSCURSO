...
function FormularioRegistro2(){
    ...
    const handleSubmit = (event) => {
      ...
      if(!/^(?=.*[A-Z])(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/.test(contraseña)){
        ...
        if((telefono.length !== 7 && telefono.length !== 9) || (telefono.length === 7 && !/^[3456]/.test(telefono)) || (telefono.length === 9 && !/^/.test(telefono))){
          alert('El telefono debe tener 7 digitos y comenzar con 3,4,5 o 6, o tener 9 digitos.');
          return;
      }
...
export default FormularioRegistro2;